...
# --- Logic to get NEXT Serial Number & Fix CSV Errors ---
def get_db_and_sl_no():
    # Columns list exactly as per our new logic
    headers = ['SL_No', 'Date', 'Farmer_Name', 'Calc_Type', 'Bags', 'Extra_KGs', 'Input_Rate', 'Gross_Total', 'Advance_Paid', 'Net_Payable']
    
    if os.path.exists(DB_FILE):
        try:
            df = pd.read_csv(DB_FILE)
            # Check if headers match (Advance_Paid unda leda ani)
            if list(df.columns) != headers:
                os.remove(DB_FILE) # Error unte delete chesi fresh start chestundi
                return 1
            if not df.empty:
...
# --- UI Setup ---
st.set_page_config(page_title="Rice Mill Pro Billing", layout="wide")
...
with col_entry:
    ...
    current_sl = get_db_and_sl_no() # Auto-Fix and Get Sl No
    
    calc_type = st.selectbox("Select Procurement Logic:", ("75kg Sum", "100kg Quintal Sum"))
    f_name = st.text_input("Farmer Name:")
    f_phone = st.text_input("WhatsApp Number:")
...
with col_preview:
    ...
    if generate_btn and f_name:
        ...
        <div class="receipt-card">
            ...
            <p><b>Farmer:</b> {f_name.upper()} | <b>Bill No:</b> {current_sl}</p>
            <hr>
        ...
        with c_pdf:
            ...
            st.download_button("⬇️ Download PDF", pdf_bytes, f"Bill_{current_sl}.pdf")
        with c_wa:
            if f_phone:
                msg = f"*PADDY BILL*\nFarmer: {f_name.upper()}\nAdvance: ₹{advance_paid:.2f}\n*Net: ₹{net:,.2f}*"
                st.markdown(f'<a href="https://wa.me/{f_phone}?text={msg.replace(" ", "%20")}" target="_blank" style="text-decoration:none;"><button style="width:100%; background-color:#25d366; color:white; border:none; padding:10px; border-radius:5px; font-weight:bold; cursor:pointer;">WhatsApp Share 📲</button></a>', unsafe_allow_html=True)
...
if os.path.exists(DB_FILE):
    try:
        ...
        st.warning("⚠️ Column mismatch found. Refresh the page to auto-fix!")
