...
# --- Logic to get the NEXT Serial Number ---
def get_next_sl_no():
...
with col_entry:
    ...
    st.info(f"📋 **Next Bill SL No: {current_sl}**")
...
if os.path.exists(DB_FILE):
    ...
    if not df.empty:
        ...
        st.download_button("📥 Download Full History CSV", csv_data, f"Rice_Mill_History_{datetime.now().strftime('%Y%m%d')}.csv", "text/csv")
        
        with st.expander("🗑️ Delete a Record (Row Delete)"):
            delete_id = st.number_input("Enter Bill No to delete:", min_value=1, step=1)
